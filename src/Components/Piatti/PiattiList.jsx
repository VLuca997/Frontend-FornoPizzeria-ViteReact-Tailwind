...
import PiattiCard from './PiattiCard';
...
const PiattiList = () => {
  ...
  useEffect(() => {
    setPiatti(Data.Piatti); // Carica i dati dei primi piatti
  }, []);
  ...
  const filteredPiatti = piatti.filter((piatto) => {
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-wrap justify-center">
        {filteredPiatti.map((piatto) => (
          <PiattiCard key={piatto.nome_prodotto} piatto={piatto} />
        ))}
      </div>
    </div>
  );
